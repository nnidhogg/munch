 ...
 * `logos::Logos`, marks the enum that follows, and the rest of the file, its uses, functions, impl blocks, comments and
 * doc comments, is skipped along with strings and character literals, so that nothing inside them is mistaken for an
 * attribute. The specification is named by the line of that derive. On the enum, `#[logos(...)]` attributes carry `skip
 * "regex"` and `skip("regex", callback, priority = n)`, each a discarded rule, in file order and before the variants as
 * logos orders them; `subpattern name = "regex"`, a definition, referenced from a pattern as `(?&name)`; and the other
 * keys logos 0.15.1 knows, `crate`, `error`, `export_dir`, `extras`, `source` and `type`, which are recorded as options
 * and say nothing about the token set, `export_dir` among them though the crate takes it only with its `debug` feature
 * on, any other key, a later crate's `utf8` among them, being refused as the crate refuses it, as is an entry after a
 ...
 * written or none, `Skip`, `Result<Skip, E>` and the `Skip` arms of `Filter` and `FilterResult` discard the match,
 * `()`, `bool`, `Option<()>`, `Result<(), E>` and the `Emit` and `Error` arms leave the variant's token or an error at
 ...
 * defines, before or after the enum, in an impl block or a module, is read by that function's return type, `Skip` or
 * `Result<Skip, E>` a skip, `Filter`, `FilterResult` and the enum, `Self` in one of the enum's impl blocks, decided by
 * the body, and any other type a payload; a closure is read by every result its body produces, the tail expression,
 * each `return`, the branches of an `if` and the arms of a `match`, through the statements before them, each of which
 * must be visibly `Skip`, `Filter::Skip`, `FilterResult::Skip` or `Ok` of one, a constructor of the enum, or `Some`,
 * `None`, `Ok`, `Err`, `true`, `false`, a literal, `()`, `Filter::Emit`, `FilterResult::Emit` or `FilterResult::Error`,
 * each read against the payload as the crate converts it. Every name in a type, a path or a result is read as the file
 * binds it: a `use` import or rename, `use logos::Skip as Drop`, a `type` alias, `type Discard = Skip`, the path
 * `#[logos(crate = ...)]` gives the crate and an item the file defines, so that a local `struct Skip` is a payload and
 * not the crate's `Skip`, a local `fn skip` is read by its type in place of the crate's function, and a constructor of
 * the enum must name one of its variants, `T::make(0)` being a call whose result is out of sight; a bare `Skip`,
 * `Filter`, `FilterResult` or `skip` the file binds no other way is the crate's, since the import bringing it in may be
 * a glob. Results that all skip discard the rule and results that all emit one variant make the rule that variant's; a
 * function the file does not define, a result the text does not show, such as `lex.slice().parse().ok()`, results that
 ...
 * name, as is a function declared without a body, whose use of the lexer is out of sight. The callback's text is the
 * rule's action. logos has no start conditions, so no rule names one and every rule is active in INITIAL.
 *
