 ...
 * stands in, wherever the two stand, as re2c has it. The end rule `$`, `<!c>` setup rules and the entry rule `<>` are
 * not tokens, a setup or entry rule whose code returns refused since re2c runs that code before any rule's own action,
 * and are skipped. A scanner's rules name conditions or name none, never both: one holding a rule of each kind, the
 * rules a `!use:` directive brought in counted with its own, is refused as re2c refuses it, which cannot mix conditions
 * with normal rules, at the first rule naming none, and an end rule `$` naming none alone among rules naming one in the
 ...
 * the line its opener is on, the opener ending at a blank, a newline, a colon and a name or the close as re2c ends it,
 * so that an opener run into a letter, `re2cx` after the bang, is refused as re2c refuses it, with the definitions and
 * configurations in force when it closed; a block holding only definitions or configurations feeds the ones after it,
 * while a local block's, a rules block's and a use block's stay in them. A definition another block uses is translated
 * again there, out of the regex as written, under that block's own flags, since re2c compiles it at every point of use:
 * `point = [^];` written where the encoding was ASCII admits one byte in its own block and a whole code point in a
 * block that turns UTF-8 on, and a flex-style definition is read under the flex syntax wherever it is used. A
 * definition another block declared that this block's flags cannot read is refused where a rule of this block reaches
 * it, directly or through the definitions the rule names, and nowhere else, since that is where re2c would compile it:
 * one no rule reaches is left out, and so is an alias of it that no rule names; one the block itself declares is read
 * where it stands and refused there. A use block, the one opened `!use:re2c[:name]`, takes the rules block of that
 * name, or the most recent one, named or not, when it names none, and the name in its opener is the block it uses and
 * no name of its own. A used block is read again where it is used, under the flags in force there, since re2c compiles
 * its regexes at every point of use: one rules block can be a scanner under one encoding and another under another, and
 * a configuration of the using block governs the rules it takes as well as the ones it writes. re2c declares no
 * conditions, so the ones a block's rules name are that scanner's, each exclusive.
 *
 ...
 * @param includes How a file the code includes by a quoted name is reached, whose macros are read as the file's
 *        own; none unless given, under which a quoted include is refused by name, its definitions being out of sight.
