 ...
 * runs to the end of the first line holding `%}`, with no comment or literal read inside it, and its code is what
 * stands between the `%{` and the `%}`, the rest of the `%}` line dropped as flex drops it; and a `|` action is its
 ...
 * end of input is not a token, once a `|` rule above has taken their action, checked then as any action is. Whether an
 * action returns a token is read by returned(). After the file's rules stands flex's default rule, the one it adds once
 * the section is read, which matches one byte where no rule of the file's does and echoes it: read as the rule `.|\n`
 * in every start condition, at the lowest priority and returning nothing, so that such a byte is a one-byte discarded
 * token, with the line the rules section ends on; `%option nodefault` drops it, since under it such a byte stops the
 * scanner with a fatal error, which is what a token set answers of itself where no rule matches, and `%option default`
 * restores it, the last word naming it deciding as with every setting. A flex file is one scanner, so the list returned
 * holds one specification, as every reader returns the scanners of its file.
 *
 * An option that changes what a rule matches beyond what the pattern parser reads is refused by name rather than
 * recorded and ignored: under `%option lex-compat` and `%option posix-compat`, which are flags of their own and either
 * of which suffices, a counted repetition binds the whole expression before it, so that `ab{3}` matches "ababab"; and
 * `%option 7bit`, as well as a `full` or `fast` table with the equivalence classes off, leaves flex building its tables
 * over the 128 bytes of ASCII and refusing outright a pattern that names a byte above 127. So is an action that moves
 * the bounds of its match or reruns it, which the token language has no place for, named with its line: `yymore()`,
 * which appends the next match to this one; `REJECT`, which drops the match for the next rule's; `yyless()`, which
 * gives the end of the match back to be matched again; `unput()`, which pushes a byte onto the input; and `input()` or
 * `yyinput()`, which consume bytes no rule matched; and `yyterminate()`, which ends the scan with no token. The action
 * is read as C reads it, token by token as c_tokens() reads them, a comment or a literal holding none of them, a call
 * being a whole word whose next token is a parenthesis, whatever blanks or comments stand between, through a member of
 * anything alike, `this->yyinput()` as a C++ scanner writes it, `self->yyinput()` through an alias the text does not
 * resolve and `(self->yyinput)()` parenthesised, and `REJECT` a whole word in capitals, as flex takes it; an action
 * naming one of them apart from a call and a member, `(input)()`, a pointer taken to it or a declaration of its own by
 * that name, is refused too, since what it does with the name is out of sight; a `|` line is taken unread, as flex
 ...
 * @param includes How a file the copied code includes by a quoted name is reached, whose definitions are read as
 *        the file's own, `YY_USER_ACTION` and `YY_BREAK` among them; none unless given, under which a quoted include
 *        is refused by name, its definitions being out of sight.
