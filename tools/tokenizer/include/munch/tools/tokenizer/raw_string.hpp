 ...
 *
 * The closing sequence must repeat the opening delimiter, and since C++ bounds that delimiter to 16 characters
 * the construct is regular in principle; a DFA covering every possible delimiter is simply impractical to build.
 * The limit is engineering, not computability, so it is scanned by hand. A driver recognizes the `R"` prefix with
 * its lexer, calls this scanner on the tokenizer's input, and seeks past the returned length; see
 * Tokenizer::seek() and Tokenizer::input().
