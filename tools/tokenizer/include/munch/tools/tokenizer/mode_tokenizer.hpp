...
{
/**
 * @brief Wrapper that turns core::Mode_lexer into a sequential token stream.
 *
 * Returns tokens in order as matched by the active mode's lexer, without additional processing.
 *
...
public:
    ...
    [[nodiscard]] Result_t<T> next()
    {
        ...
        // The lexer advances the stack where the grammar says so, and leaves it where the caller drives it.
        const auto [token, consumed]{lexer_.template tokenize<T>(view.cbegin(), view.cend(), stack_)};
...
#endif // MUNCH_TOOLS_TOKENIZER_INCLUDE_MUNCH_TOOLS_TOKENIZER_MODE_TOKENIZER_HPP
