...
- `analyze_r6_mechanism.py` emits the coverage-mechanism figures: the overhang identity checked per row,
  ...
  geometry, and the bare row's within-row decomposition.
- `test_analyze_r6.py` proves the audit fail-closed by execution: a byte-identical pristine baseline, a
  mechanism-emission baseline, and sixty-three staged archive corruptions that must each be rejected by
  the guard built for them. Invoked as `test_analyze_r6.py --prove-detection` it proves its own
  detection as well, disarming one corruption so the archive reaches the analyzer unchanged and exiting
  0 only when the ordinary run reports the resulting acceptance as a failure.
