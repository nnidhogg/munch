...
unrestricted. It ships as a query only: the library's planner and every measurement here use the exact condition, so
using it means planning boundaries yourself. Splitting at exact certificates in the munch lexer library~\cite{munch}
reaches 92.6--95.3\% parallel efficiency at eight threads on a 512\,MiB dense corpus beyond last-level cache, and a
...
eight-chunk efficiency from 95\% to 93\% at 512\,MiB and from 80\% to 68\% at 1\,MiB, the gap falling from twelve points
at 1\,MiB to about three at 512. The two- and four-chunk figures move by two points or less at 128 and 512\,MiB and by
more at the smaller sizes. One hypothesis is SMT: with eight threads free on 32 logical processors, some pairs share a
physical core. We do not claim it, and cannot from this data. For each benchmark revision there is one run per
...
recorded during the scan. What the two runs support is narrower: on these runs the restricted CPU set had the higher
eight-chunk efficiency, though not always the higher absolute eight-chunk throughput, and certified splitting delivered
...
the certificate is non-empty the measured gain is 92.6--95.3\% parallel efficiency at eight threads against the same API
driven with one chunk, on a 512\,MiB dense corpus that does not fit in cache, and $3.46$--$3.94\times$ end to end at
four threads. Both are conventionally rounded ranges across the two benchmark revisions on one machine, and neither is
a confidence interval: the per-scenario best-to-worst
