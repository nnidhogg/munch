...
a property a compiler checks. \end{abstract}
...
discarded set. Deriving the certificate is a linear-time sweep of the transition table at construction time, and the
same sweep decides which certified bytes can occur in valid input at all. That makes the applicability question
...
non-empty the measured gain is 92.6--95.3\% parallel efficiency at eight threads on a restricted CPU set, against the
same API driven with one chunk, on a 512\,MiB dense corpus that does not fit in cache, and $3.46$--$3.94\times$ end to
end at four threads. Both are conventionally rounded ranges across the two benchmark revisions on one machine, taken
in the pinned placement; the unpinned placement reaches 88.5\% at eight chunks on the earlier revision. Neither is a
confidence interval either: the per-scenario best-to-worst spreads behind them reach 9\% and 15\% on the two
eight-chunk rows at 512\,MiB. The wider end-to-end range comes from the serial baseline used as its denominator: at
512\,MiB the multi-chunk rows move far less between the revisions than that baseline does;
