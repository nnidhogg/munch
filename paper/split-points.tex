...
\item a second condition, weakening equality to hold only after the tokens a caller discards are deleted:
\emph{sound and strictly more permissive but conservative rather than exact}, decided from the same tables and
answered by a second constant-time one-bit query. It recovers newline for the conventional C-like and JSON
tokenizations that certify nothing exactly, and ships as a query rather than in the planner, so a caller wanting it
places the boundaries itself (Section~\ref{sec:modulo});
...
that a C-like language with block comments cannot be chunked at line boundaries, and the certificate says so.
...
splitting on every candidate byte the corpus exercises, which is a declared set of sixteen covering whitespace and the
structural characters these token sets use, not all 256. The conservatism measured in Section~\ref{sec:decide}
