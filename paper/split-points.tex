...
\item a controlled applicability study of eleven token sets under both conditions, showing where each property
holds and where a single token kind destroys it, which grounds the folklore that source text can be split at line
boundaries (Section~\ref{sec:applicability}).
\end{itemize}
\end{samepage}
...
rejected in practice because generated JSON may contain none~\cite{li2021plex}. Lua fails for a stronger reason than
any certificate could address: its long-bracket delimiters \code{[=$^n$[} and \code{]=$^n$]} must agree on $n$, so the
...
missing is a way to decide, from the token set alone, whether every relevant occurrence of a proposed byte is safe to
cut immediately before.
...
None of these methods asks the token set's own automaton which raw-input bytes are safe at every occurrence. Lexer
generators come closest: re2c analyses the compiled automaton to reject rules in which one user-declared end-of-input
sentinel may occur before a lexeme ends~\cite{re2cdocs}. That is a terminal-in-lexeme test for a declared byte, a
safe-after property under the model used here, rather than a derivation of the safe-before set, and
Section~\ref{sec:related} shows the two properties are incomparable.
...
those choices leave implicit, namely a sound grammar-only test of whether every relevant occurrence of a given byte is
safe to cut immediately before. Among the surveyed generators, re2c~\cite{trofimovich2020re2c} validates a
...
one declared byte; under the completely-tokenizable-input model used here it is a safe-after property, and it is
genuine static analysis of the compiled automaton rather than a convention. It is incomparable with the certificate
...
as the exact analysis, adding a backward closure, a second sweep of the stored table, and ordered-set membership costs
logarithmic in the declared discarded set. The recovered cases include ones that matter in practice: the conventional
C-like token set studied here and a JSON lexer both gain newline without any change to their token definitions, which
removes a token set redesign that the exact method had imposed on its users. The relaxation is conservative in
general, rejecting roughly one exercised pair in four that had no counterexample through length eight, while agreeing
with brute-force splitting on every candidate byte tested for the token sets studied.
