...
appended to one `observations.csv` and separated by its `run` column. Backs the two mode tables in `docs/usage.md`. The
benchmark records the commit and whether the tree was dirty on every CSV row, so a row detached from this directory
still says which tree produced it.
