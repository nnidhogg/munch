...
Full output of `tools/benchmark/collect.sh` on an AMD Ryzen 9 9950X3D running Ubuntu 26.04, `performance` governor, over
1, 16, 128 and 512 MiB corpora, each with a clean tree.
...
All three measured trees carry an annotated tag, since `master` is rewritten freely: `benchmark/split-points-2026-07`
for the Intel run's `c0e2fb6`, `benchmark/split-points-2026-08-run1` for `7d067c2`, and `benchmark/split-points-2026-08`
for `141b12e`.
...
In run 1 the CSV stored each pass at the default six significant digits, so a statistic recomputed from it could land
0.1 MiB/s away from the summary, which computes from the timings in memory; that affects three of the 40 pinned groups
and seven of the 40 unpinned ones. Run 2 writes `max_digits10`, and every statistic in both its archives recomputes from
the CSV exactly.
