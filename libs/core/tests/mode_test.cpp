...
TEST(Mode, The_winning_nullable_token_carrying_an_action_is_rejected)
{
    Mode_builder builder;
...
TEST(Mode, A_negative_or_unrepresentable_id_is_rejected)
{
    ...
    // The unsigned maximum reaches no negative check, and the id + 1 that sizes the row wraps to zero, so the next
    // index is out of bounds on an empty vector. This crashed before it was checked.
    constexpr auto unrepresentable{static_cast<std::size_t>(-1)};
...
TEST(Mode, Far_apart_token_ids_match_and_carry_their_actions)
{
    Mode_builder builder;
...
TEST(Mode, The_two_drivers_agree_for_action_counts_to_three_at_dense_and_sparse_token_ids)
{
    ...
    // once, returns through y each time, and ends on z in mode 0, so both drivers consume it whole. Zero actions is
    // the count that leaves mode 0 with nothing to act on, which is the batch driver's separate no-action pass.
