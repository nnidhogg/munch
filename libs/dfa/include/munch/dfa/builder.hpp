 ...
 *
 * Allows incremental construction of a DFA by adding states, transitions, and accept states. The identifiers are
 * the caller's: next_state() hands out a dense sequence from zero, but add_transition() and add_accept_state()
 * take any identifier, so a DFA built here is densely numbered only where every identifier came from
 * next_state(), as the subset construction's do, and Dfa::state_count() is the span of the identifiers used
 * either way. A caller naming its own identifiers keeps the highest of them at most the largest std::size_t less
 * two, so that the span, one past it, and the span of the automaton unroll_start() returns, one past that again,
 * are both representable, which is what unroll_start() asks of a definition; the Simulator asks less of it still,
 * a span its table entries index, below 2^32 - 1.
 */
class Builder
{
public:
