...
- flex: definitions, `%s` and `%x`, `%option`, `%{ %}` and `%top{ }` blocks, start-condition scopes `<s>{ }` with
  indented rules, `|` shared actions, actions running to the first line end at which their braces balance, `<<EOF>>`
  rules (not tokens). Refused: `^` and `$` anchors and `/` trailing context, which condition a match on its context
  and are no token language, and `%option case-insensitive`, which is not modelled.
